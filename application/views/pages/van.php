<div class="container">
  <header class="labels">
    <h1>The VanHorns</h1>
  </header>
</div>
<div class="container">
  
  <section class="flex-one item">
    <h1 class="page-title">Beginnings</h1>
    <article>
      <h3>Denmark!</h3>
      <p>
        In the year 1640 or there abouts, in Jutland Denmark, a man named Matthys Corneliussen was born. Nothing is known about his parents or his life in general there. He immigrated to the US, arriving at New Netherlands in 1663.  This is where the VanHorn family begins. Had he never immigrated, we would not be here today.
      </p> 
      
      <p>
        He married Fytje Sophia Brouwer(1664-1709) on February 20, 1692. They had two children during their marriage. He died in 1703 in Monmouth County, New Jersey, at the age of 63. It should be noted the date of his dying is not known as a fact.
      </p>
      <p>
        The VanHorns are not like the Bosticks, in that there are no 2 long standing familys that lead to our Dad. The Bosticks have the Reeds, Sholes(Mom's Grandmother), Stoddards and the Bostick families, while the VanHorns have, the VanHorns. Of course there are still some longer lineage in Dad's family. The Clays(His grandmother), the Hillyers and the Roricks. But this family doesnt really open up to a huge array of names like Mom's family does. And of course, Mom's family goes back much further then Dad's does.
      </p>
    </article>

  </section>

  <section class="flex-two item">
    <h1>Details</h1>
    <article>
      <h3>Matthy's Sons</h3>
      <p> Matthys had 2 sons, Abraham(1699-1759) and Cornelius(1695-1744). </p>
        <p>Abraham Van Horne was baptized on 15 January 1699 at Old First Dutch Reformed Church, Brooklyn, Kings Co., New York; sponsors Niklaes Brouwer and his wife Jannetje.<sub>2</sub> </p><p> Cornelis Van Horne was baptized on 4 May 1695 at Old First Dutch Reformed Church, Brooklyn, Kings Co., New York; sponsors Thomas Lammerse, Elizabeth Thomas.Cornelius was a Captain in the Kings Militia<sub>3</sub></p>       

      <p>Based on my DNA testing, we are direct descendants of Matthys and his son Abraham.</p>
      <p>When Matthys moved here, the government required a surname. It is thought that Matthys came from one of the two cities named Horne in northern and southern Jutland. So the two sons used the surnames Van Horne. This was the first use of the name VanHorne ['see Notes'] in our part of the family.</p>
      
      <p>Abraham, who is our 6th great-grandfather, Started a tavern in the place now known as Whitehouse, Hunterdon New Jersey. The town is called Whitehouse becuase his tavern was painted white.</p>
      <blockquote>
        <p>In 1722 Abraham sold his half of the property and purchased 490 acres of land in Readington Township, Hunterdon Co., New Jersey through which ran Rockaway Creek. This land was in 490 acres of “L” shape. Here he developed a grist mill and saw mill on the banks of the creek. About 1750 Abraham Van Horne built the first tavern in the area. It was on the road from Clinton to Somerville where it crossed Rockaway Creek (now route 22). Because the building had white plastered walls it became known as the "White House". The name also was given to the village which sprang up around the tavern. It's location on this early trail immediately led to it's being a favorite stopping place for travelers and later for stage coaches of the Easton-New Brunswick Turnpike. By the time of the Revolution it's popularity had been well established. The homestead built about 1757 by Abraham Van Horne(Jr.) is about one hundred yards farther down Rockaway Creek from the site of the tavern and is still occupied (March 1976).</p>

        <p>After conducting Antjie Van Horne's funeral service, Rev. Muhlenberg made this note: "17 Dec 1759. Had to bury the deceased wife of Abraham Van Horne, who himself died only recently. Both died of a contagious kind of pox. The youngest son, his wife, and his negroes are still sick with the pox. I preached at the home in English on Psalm 90:12 and read the customary prayers in English at the grave. I also prayed for the sick in the home." <sub>4</sub></p>
      </blockquote>

    </article>
  </section>
  <section class="flex-three item">
    <h1>Notes</h1>
    <article>
      <h3>Matthys</h3>
      <ul>
        <li><blockquote>There were three immigrants to New Netherlands whose descendants took some version of the Van Horne surname. The three men were Jan Cornelissen, from Hoorn, North Holland; Christian Barentsen, probably from Hoorn, Terschelling Island, Friesia; and Matthys Cornelissen from Jutland(Denmark). These men were not related, and had absolutely nothing to do with one another after coming to North America.<sub>1</sub></blockquote></li>
        <li>New Netherlands became what is now known as New York City when the British showed up and forced the city to surrender.</li>        
      </ul>     
    </article>
    <article>
    <h3>VanHorn Names</h3>
      <p>This is by far not a complete list, again just verified ancestors. The names of husbands and wives</p>
      <?php foreach($famnames as $row)//From pages/van
        {
          echo "<li>$row->male - $row->female</li>" ;
        }
        ?>
    </article>
  </section>
</div>
<div class="container">
  <footer class="flex-four">
    <h3>Footnotes</h3>
    <ul>
    <li><sub>1</sub> This information is from <a href="http://archiver.rootsweb.ancestry.com/th/read/NJHUNTER/2010-09/1283444071" rel="external">Marleen Van Horne</a> Van Horne-DNA and the origin of Matthys Cornelissen <li>
    <li> <sub>2,3</sub> This information comes from <a href="http://www.frostandgilchrist.com/getperson.php?personID=I19609&tree=frostinaz01">Frost, Gilchrist and Related Families</a></li>
    <li><sub>4</sub>
      Information on Abraham VanHorne taken from <a href="http://wc.rootsweb.ancestry.com/cgi-bin/igm.cgi?op=GET&db=kings-highway&id=I81737" rel="external">Kings Highway</a></li>
  </ul>
  </footer>
</div>
